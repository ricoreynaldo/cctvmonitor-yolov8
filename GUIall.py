...
def process_frame(frame, start_time, ground_truth_file=None):
    ...
    # Hasil deteksi menggunakan YOLOv8
    results = model(frame)
    detections = results[0].boxes.data.cpu().numpy()

    # Membaca file ground truth (jika tersedia)
    ground_truth = []
    if ground_truth_file:
        try:
            ...
            ground_truth = []  # Jika file gagal dibaca, gunakan list kosong

    # Menggambar bounding box ground truth
    ...
    for det in detections:
        ...
        confidence = conf
        label_text = f"{label_translation.get(label, label)} ID-{object_id} ({confidence:.2f})"
        ...
        cv2.rectangle(frame, (x1, y1 - label_height - 5), (x1 + label_width, y1), color, -1)  # Background label
        cv2.putText(frame, label_text, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

    # Menghapus objek yang sudah expired
    expiration_time = 2.0
...
class DetectionApp(QMainWindow):
    def __init__(self):
        ...
        self.video_widget.setFixedSize(1200, 675)
        ...
        self.object_count_label.setObjectName("object-count-label")
    ...
    def create_button_handler(self, key):
        def handler():
            ...
            # Debugging untuk memeriksa kunci dan URL yang diambil
            if key in urls_dict:
    ...
    def update_frame(self, frame=None, total_objects=0):
        ...
        self.object_count_label.setText(f"Jumlah objek terdeteksi: {total_objects}")
